...
def main(argv: list[str] | None = None) -> None:
    ...
    for w in range(5):
        tasks.append(Task())
    tasks[2].highlight(False)
    ###

    def scroll_selection(selected,delta: int) -> selected:
        tasks[selected].highlight(False)
        selected = selected+delta
        if selected > len(tasks):
            selected = 0
        tasks[selected].highlight(True)
        return selected
    ...
    with ptg.WindowManager() as manager:
        ...
        body = ptg.Window("Tasks")
        for task in tasks:
            body+=task
        manager.add(body, assign="body")
        scroll_selection(0,0)
        selected = 0
        body.bind(ptg.keys.DOWN,lambda body, _: scroll_selection(selected,1))



    ptg.tim.print("bye bye!")
